...
#include <ctime>                            // Only for debuging purposes
...
private:
    ...
    void SeedsDependentMatrices( const int &m , const int &n , const int &xf , const int &xb , SparseMatrix<double> &Is , VectorXd &b ) {
    ...
    void GraphLaplacianMatrixSquare( const int &m , const int &n , Eigen::SparseMatrix<double> &L ) {
    ...
    void ComputeLinearSystem( const int &m , const int &n , const SparseMatrix<double> &Is_L , const VectorXd &b , VectorXd &X ) {
    ...
    void AssignLabels( const int &m , const int &n , const int &xf , const int &xb , const VectorXd &X , Mat_<double> &Y ) {
        // Threshold to assign labels
        double threshold = ( ( xb + xf ) / 2 );
